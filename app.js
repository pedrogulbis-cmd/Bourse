...
function sampleCandidates(fullList, n){
  ...
  const withAge = fullList.map(item=>{
    const cached = cacheGetFund(item.symbol);
...
// ---------------------------------------------------------------
// Fundamentals fetch per symbol (5 appels, cachés 24h)
// ---------------------------------------------------------------
async function fetchFundamentals(symbol){
  ...
  const data = {q, r, km, pc, cf, fetchedAt: Date.now()};
...
function rawToRecord(company, fund){
  ...
  return {
    ...
    mcap: mcap,
    // ratios bruts, nettoyés : valeurs non-positives traitées comme manquantes pour les facteurs "moins cher = mieux"
...
function estimateBudget(){
  ...
  const perCandidate = 5; // quote, ratios-ttm, key-metrics-ttm, stock-price-change, cash-flow-statement-ttm
...
async function runScreening(){
  ...
  try{
    ...
    const funds = await runPool(candidates, 6, async (c)=>{
      const f = await fetchFundamentals(c.symbol).catch(()=>({q:null,r:null,km:null,pc:null,cf:null}));
      done++;
    ...
    if(state.strategy === "all_stocks_growth"){
      ...
      const growths = await runPool(records, 6, (c)=>fetchEpsGrowth(c.symbol).catch(()=>null));
    ...
    progressTxt.textContent = `Terminé — ${requestCount} requêtes FMP utilisées (Wikipedia hors quota).`;
  }catch(err){
    console.error(err);
...
function renderResults(){
  ...
  meta.textContent = `Échantillon analysé : ${state.lastRunMeta.poolCount} / ${state.lastRunMeta.universeCount} titres de l'univers réel · ${countryLabel} · ${new Date(state.lastRunMeta.ts).toLocaleString('fr-FR')}`;
