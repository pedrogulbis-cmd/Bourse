...
function sampleCandidates(fullList, n){
  const now = Date.now();
  const prefix = state.dataSource === "yahoo" ? "yahoo:" : "fmp:";
...
// ---------------------------------------------------------------
// Source de données Yahoo Finance (secours gratuit, sans clé) —
// Yahoo ne renvoie pas d'en-têtes CORS pour les requêtes navigateur,
// on passe donc par un proxy CORS public. Plusieurs proxys sont
// essayés dans l'ordre en cas d'échec/indisponibilité de l'un d'eux.
// ---------------------------------------------------------------
const CORS_PROXIES = [
...
async function yahooFetchJSON(targetUrl){
  ...
  for(const wrap of CORS_PROXIES){
    try{
      const res = await fetchWithTimeout(wrap(targetUrl), undefined, 10000);
      if(!res.ok) throw new Error("HTTP " + res.status);
      return await res.json();
    }catch(e){ lastErr = e; }
  }
  throw new Error("Tous les proxys CORS ont échoué (" + (lastErr ? lastErr.message : "?") + ")");
}

function yraw(field){ return (field && typeof field.raw === "number") ? field.raw : null; }

async function fetchFundamentalsYahoo(symbol){
  const cached = cacheGetFund("yahoo:"+symbol);
  if(cached) return cached;

  const qsUrl = `https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(symbol)}?modules=summaryDetail,defaultKeyStatistics,financialData,price,cashflowStatementHistory`;
  const chartUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=1y&interval=1d`;

  let qsData=null, chartData=null, qsErr=null;
  try{ qsData = await yahooFetchJSON(qsUrl); }catch(e){ qsErr = e; }
  try{ chartData = await yahooFetchJSON(chartUrl); }catch(e){ /* momentum restera à 0 */ }

  if(qsData && qsData.quoteSummary && qsData.quoteSummary.error) qsErr = new Error(qsData.quoteSummary.error.description || "symbole introuvable");
  if(qsErr) throw qsErr;

  const result = qsData?.quoteSummary?.result?.[0] || {};
  const summaryDetail = result.summaryDetail || {};
  const keyStats = result.defaultKeyStatistics || {};
  const financialData = result.financialData || {};
  const priceMod = result.price || {};
  const cashflowHist = (result.cashflowStatementHistory && result.cashflowStatementHistory.cashflowStatements && result.cashflowStatementHistory.cashflowStatements[0]) || {};

  const mcap = yraw(summaryDetail.marketCap) ?? yraw(priceMod.marketCap) ?? 0;
  const pe = yraw(summaryDetail.trailingPE) ?? yraw(keyStats.trailingPE);
  const pb = yraw(keyStats.priceToBook);
  const ps = yraw(summaryDetail.priceToSalesTrailing12Months);
  const evEbitdaMult = yraw(keyStats.enterpriseToEbitda);
  const divYield = yraw(summaryDetail.dividendYield) ?? 0;
  const opCashflow = yraw(financialData.operatingCashflow);
  const pcf = (opCashflow && opCashflow>0 && mcap>0) ? (mcap/opCashflow) : null;
  const buyback = yraw(cashflowHist.repurchaseOfStock); // négatif = cash dépensé, souvent annuel (pas TTM exact)
  const buybackYield = (buyback!==null && mcap>0) ? Math.max(0, -buyback)/mcap : 0;
  const epsGrowth = yraw(financialData.earningsGrowth);

  let mom3 = 0, mom6 = 0;
  try{
    const cr = chartData.chart.result[0];
    const closes = cr.indicators.quote[0].close.filter(v=>v!==null && v!==undefined);
    const n = closes.length;
    const last = closes[n-1];
    const c3 = closes[Math.max(0, n-64)];
    const c6 = closes[Math.max(0, n-127)];
    if(c3) mom3 = ((last/c3)-1)*100;
    if(c6) mom6 = ((last/c6)-1)*100;
  }catch(e){ /* pas d'historique -> momentum neutre à 0 */ }

  const data = {
    price: yraw(priceMod.regularMarketPrice) ?? null,
    exchange: priceMod.exchangeName || "—",
    mcap,
    pb: (pb!==null && pb>0) ? pb : null,
    pe: (pe!==null && pe>0) ? pe : null,
    ps: (ps!==null && ps>0) ? ps : null,
    pcf: (pcf!==null && pcf>0) ? pcf : null,
    ebitdaYield: (evEbitdaMult!==null && evEbitdaMult>0) ? (1/evEbitdaMult) : null,
    divYield,
    shareholderYield: divYield + buybackYield,
    mom3, mom6,
    epsGrowth,
    source: "yahoo",
    fetchedAt: Date.now(),
  };
  cacheSetFund("yahoo:"+symbol, data);
  return data;
}
...
async function fetchFundamentals(symbol){
  return state.dataSource === "yahoo" ? fetchFundamentalsYahoo(symbol) : fetchFundamentalsFMP(symbol);
}

async function fetchEpsGrowth(symbol){
  if(state.dataSource === "yahoo"){
    // déjà récupéré dans fetchFundamentalsYahoo (financialData.earningsGrowth)
    const cached = cacheGetFund("yahoo:"+symbol);
...
function updateBudgetUI(){
  ...
  if(state.dataSource === "yahoo"){
    ...
    warn.textContent = "Yahoo Finance : pas de clé, pas de quota fixe — mais via un proxy public gratuit, donc plus lent et parfois instable.";
    return;
  }
...
async function runScreening(){
  if(state.dataSource === "fmp"){
    ...
    if(!apiKey){
      toast("Entrez d'abord votre clé API Financial Modeling Prep, ou passez sur la source Yahoo Finance (réglages avancés).");
      return;
    }
    setApiKey(apiKey);
    updateKeyStatus();
  ...
  try{
    ...
    let done = 0;
    const concurrency = state.dataSource === "yahoo" ? 3 : 6; // proxys publics = plus prudent
    ...
    if(withData.length === 0){
      if(state.dataSource === "fmp" && quotaErrorCount > records.length){
        toast(`Quota FMP journalier dépassé (${quotaErrorCount} appels rejetés sur ${requestCount}). Le quota gratuit (250/jour) se réinitialise le lendemain — réessaie demain, réduis l'échantillon, ou passe sur la source Yahoo Finance (réglages avancés).`);
      }else if(state.dataSource === "yahoo"){
        toast(`Aucune donnée exploitable via Yahoo Finance pour cet échantillon — les proxys CORS publics sont peut-être temporairement indisponibles ou rate-limités. Réessaie dans quelques minutes, ou repasse sur FMP.`);
      }else{
    ...
    if(state.dataSource === "yahoo"){
      progressTxt.textContent = `Terminé — données via Yahoo Finance (proxy public, hors quota FMP).`;
    }else{
...
function renderResults(){
  ...
  const srcLabel = state.lastRunMeta.dataSource === "yahoo" ? "Yahoo Finance" : "Financial Modeling Prep";
...
function init(){
  ...
  document.getElementById("dataSource").addEventListener("change", (e)=>{
    ...
    document.getElementById("apiKey").closest(".field").style.opacity = state.dataSource==="yahoo" ? "0.5" : "1";
    updateBudgetUI();
  });
